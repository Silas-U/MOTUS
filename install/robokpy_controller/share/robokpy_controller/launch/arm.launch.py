...
--- External robot-description override (Motus Builder support) ---

Built-in robots (arm_type=ur5e / mycobot_280 / ...) keep working exactly
as before: the URDF comes from robokpy_controller's own
urdf/robokpy_robot.urdf.xacro, selected by the `arm_type` xacro:if
branches, and its config/controllers YAML come from robokpy_controller's
own share directory by convention.

A robot generated by `motus create project` / `motus robot add` ships
its OWN urdf(.xacro), config/robot.yaml and config/controllers.yaml
inside its OWN package (e.g. `my_robot_motus`) rather than inside
robokpy_controller. Three new, optional launch arguments let a generated
project's thin launch wrapper point this file at that package's files
directly, instead of at robokpy_controller's built-in ones:

    robot_description_path   -- absolute path to a .urdf or .xacro file
    robot_config_path        -- absolute path to a robot.yaml (same shape
                                 as config/robots/<arm_type>.yaml)
    controllers_yaml_full_path -- absolute path to that robot's
                                 controllers.yaml

All three default to '' (empty). Leaving them empty reproduces today's
behavior exactly -- nothing about the built-in-robot path changes. Set
any subset of them; unset ones still fall back to the arm_type/
controllers_yaml convention-based lookup inside robokpy_controller.

For an externally-supplied URDF, mesh URIs are expected to use
`package://<mesh_package_name>/...` where <mesh_package_name> is the
generated project's own package name -- pass it via `mesh_package_name`
so the sim/Gazebo mesh-path rewrite (see _resolve_meshes_path below)
finds the right share directory instead of assuming robokpy_controller.
...
def generate_launch_description():
    ...
    meshes_path_default = os.path.join(pkg, 'meshes')
    default_objects_config_path = os.path.join(pkg, 'config', 'objects.yaml')
    prefix = ''
    world_frame = 'world'
    ...
    def configure(context):
        ...
        # --- NEW: resolve override paths (empty string == "not set") ---
        ...
        # controllers.yaml: external absolute path wins if given, else the
        # existing convention (robokpy_controller/config/<controllers_yaml>).
        if controllers_yaml_full_path_str:
            controllers_yaml_path = controllers_yaml_full_path_str
        else:
            controllers_yaml_path = os.path.join(pkg, 'config', controllers_yaml_str)

        # robot.yaml: external absolute path wins if given, else the
        # existing convention (robokpy_controller/config/robots/<arm_type>.yaml).
        if robot_config_path_str:
            robot_config_yaml = robot_config_path_str
        else:
            robot_config_yaml = os.path.join(pkg, 'config', 'robots', f'{arm_type_str}.yaml')
        ...
        logger.info(
            f'[{ns}] arm_type={arm_type_str}, tool_type={tool_type_str}, '
            f'ik_backend={backend_str}, embed_object_catalog_plugin={embed_plugin}, '
            f'external_description={using_external_description}'
        )
        ...
        # --- NEW: build the robot_description XML from either the
        # built-in macro-based xacro (unchanged path) or an external
        # URDF/xacro supplied by a generated Motus Builder project. ---
        def _process_description(sim_value):
            if using_external_description:
                ...
                with open(robot_description_path_str, 'r') as f:
                    return f.read()
            return xacro.process_file(
                robot_xacro_file_default,
                mappings={
                    'prefix': prefix, 'arm_type': arm_type_str, 'gripper_type': tool_type_str,
                    'sim': sim_value, 'position_proportional_gain': '0.5',
                    'controllers_yaml_path': controllers_yaml_path,
                    'namespace': ns,
                },
            ).toxml()

        # --- NEW: resolve the mesh share-directory used to rewrite
        # package://<pkg>/meshes URIs for Gazebo. Built-in robots keep
        # using robokpy_controller's own meshes dir. An external
        # description declares its own package name via
        # `mesh_package_name` (defaults to robokpy_controller if unset,
        # matching today's behavior, so this is safe even if a generated
        # project forgets to pass it -- doctor should still flag that). ---
        if mesh_package_name_str:
            resolved_mesh_package = mesh_package_name_str
            meshes_path = FindPackageShare(mesh_package_name_str).find(mesh_package_name_str)
            meshes_path = os.path.join(meshes_path, 'meshes')
        else:
            resolved_mesh_package = 'robokpy_controller'
