"""
motion_planner_node.py  —  Thin ROS node

The only file that knows about ROS topics, actions, and parameters.
All business logic is delegated to focused subsystems:

  KinematicsFacade   → mp_kinematics
  WaypointManager    → mp_waypoints
  TrajectoryDispatcher → mp_trajectory
  PlanCache          → mp_cache
  JtcExecutor        → mp_jtc
  VizPublisher       → mp_viz
"""
...
from .mp_trajectory import TrajectoryDispatcher, _TrajectoryDispatchError
...
class MotionPlannerNode(Node):
    def __init__(self):
        ...
        self._pipeline_factory.register_generator(
            ...
            lambda n_samples, **kw: __import__('robokpy.pipeline', fromlist=['CartesianLineGenerator']).CartesianLineGenerator(
                ...
                base_link=self._kin.model.model.base_link,
                tip_link=self._kin.model.model.tip_link,
                n_samples=n_samples
            )
        )
    ...
    def _pending_config(self) -> TrajectoryConfig:
        """Build config from current pending state (cmd_cb modifies this)."""
        # We store pending config as a TrajectoryConfig on the node
    ...
    def _tip_link_cb(self, msg: String):
        self._kin._tip = msg.data
        self._kin._model.ik.tip_link = msg.data
    ...
    def _record_waypoint(self):
        ...
        pose = self._kin.compute_fk(q)
        wp = CartesianWaypoint(pose=pose) if self.pose_current is None else CartesianWaypoint(pose=self.pose_current)
        # Actually we have q, so use JointWaypoint for consistency
    ...
    def _add_waypoint_at_joints(self, q_target: np.ndarray) -> str:
        pose = self._kin.compute_fk(q_target)
        wp = JointWaypoint(q=q_target)
        wp_id = self._waypoint_mgr.add_with_config(wp, self._pending_config())
    ...
    async def _set_joint_target_cb(self, goal_handle):
        ...
        try:
            ...
            # Package into JTC message
            from .mp_trajectory import TrajectoryDispatcher
            traj_res = self._dispatcher._package_result(
    ...
    def _cached_plan_to_result(self, plan: CachedPlan) -> 'TrajectoryResult':
    ...
    @staticmethod
    async def _await_future_with_timeout(future: Future, timeout_sec: float):
        """Simple asyncio-compatible future await with timeout."""
        import asyncio
