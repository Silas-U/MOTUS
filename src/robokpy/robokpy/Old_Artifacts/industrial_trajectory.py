...
# =========================================================
# 1b. OPTIMIZED GENERAL ASYMMETRIC S-CURVE
# =========================================================
#
# Optimized for Motus horizon/seam planning.
#
# Main design goals:
#
#   1. Preserve exact boundary velocity/acceleration.
#   2. Avoid the old:
#
#        400 cruise candidates
#          x 24 accel candidates
#          x 40 accel bisection
#          x 40 outer headroom bisection
#
#      explosion.
#
#   3. Keep the normal rest-to-rest S-curve implementation untouched.
#   4. Use vectorized coarse search only where it actually saves work.
#   5. Replay a fully materialized profile during evaluation.
#
# The implementation intentionally remains deterministic:
# no optimizer, no scipy.optimize, no object allocation inside the
# trajectory sample loop.
# =========================================================


_GENERAL_ZERO = 1e-10
_GENERAL_FEAS_TOL = 1e-9


def _kin_ramp(v0, a0, a1, jmax):
    """Constant-jerk acceleration ramp.

    Returns:
        duration, final_velocity, displacement
    """
    da = a1 - a0

    if abs(da) <= _GENERAL_ZERO:
        return 0.0, float(v0), 0.0

    j = jmax if da > 0.0 else -jmax
    T = abs(da) / jmax

    v1 = v0 + a0 * T + 0.5 * j * T * T
    ds = v0 * T + 0.5 * a0 * T * T + j * T * T * T / 6.0

    return float(T), float(v1), float(ds)


def _kin_hold(v0, a0, T):
...
def _entry_block(v0, a0, apk, vcru, jmax):
    """Acceleration-side block.

    a0 -> apk -> 0

    Returns:
        T1, T2, T3, displacement, final velocity
    """
    if abs(apk) <= _GENERAL_ZERO:
        return None

    T1, v1, s1 = _kin_ramp(v0, a0, apk, jmax)
    T3, v3_base, s3_base = _kin_ramp(v1, apk, 0.0, jmax)

    T2 = (vcru - v3_base) / apk

    if T2 < -_GENERAL_FEAS_TOL:
        return None

    T2 = max(T2, 0.0)

    v2, s2 = _kin_hold(v1, apk, T2)
    T3, v3, s3 = _kin_ramp(v2, apk, 0.0, jmax)
...
def _exit_block(vcru, apk, vf, af, jmax):
    """Deceleration-side block.

    0 -> apk -> af

    Returns:
        T5, T6, T7, displacement, final velocity
    """
    if abs(apk) <= _GENERAL_ZERO:
        return None

    T5, v1, s1 = _kin_ramp(vcru, 0.0, apk, jmax)
    T7, v7_base, s7_base = _kin_ramp(v1, apk, af, jmax)

    T6 = (vf - v7_base) / apk

    if T6 < -_GENERAL_FEAS_TOL:
        return None

    T6 = max(T6, 0.0)

    v2, s2 = _kin_hold(v1, apk, T6)
    T7, v_end, s7 = _kin_ramp(v2, apk, af, jmax)
...
def _entry_triangle(v0, a0, apk, jmax):
    """Reduced peak-acceleration entry block."""
    T1, v1, s1 = _kin_ramp(v0, a0, apk, jmax)
    T3, v3, s3 = _kin_ramp(v1, apk, 0.0, jmax)

    return v3, s1 + s3, T1 + T3


def _exit_triangle(vcru, apk, af, jmax):
    """Reduced peak-acceleration exit block."""
    T5, v1, s1 = _kin_ramp(vcru, 0.0, apk, jmax)
    T7, v_end, s7 = _kin_ramp(v1, apk, af, jmax)

    return v_end, s1 + s7, T5 + T7


def _bisect_scalar_root(fn, lo, hi, target, iters=28):
    """Small deterministic scalar bisection.

    Used only for the unusual reduced-peak-acceleration case.
    """
    f_lo = fn(lo) - target
    f_hi = fn(hi) - target
...
def _general_entry_side(v0, a0, vcru, amax, jmax):
    """Fast entry-side resolver."""

    # Full acceleration direction.
    apk = amax if vcru >= v0 else -amax
    ...
    if block is not None:
        T1, T2, T3, s, v3 = block
        return T1, T2, T3, s, v3, apk

    # Reduced peak acceleration.
    def velocity_for_apk(a):
        return _entry_triangle(v0, a0, a, jmax)[0]

    apk = _bisect_scalar_root(
        velocity_for_apk,
        -amax,
        amax,
        vcru,
...
def _general_exit_side(vcru, vf, af, amax, jmax):
    ...
    if block is not None:
        T5, T6, T7, s, v_end = block
        return T5, T6, T7, s, v_end, apk

    def velocity_for_apk(a):
        return _exit_triangle(
            vcru,
            a,
            af,
            jmax,
    ...
    v_end, s, duration = _exit_triangle(
        vcru,
        apk,
        af,
        jmax,
...
def _general_profile_for_vcru(
    ...
    """Construct a complete profile for one cruise velocity.

    This function is deliberately centralized so the caller can retain
    the winning profile instead of recomputing it.
    """
    ...
    s_fixed = s_entry + s_exit

    if vcru > _GENERAL_ZERO:
        Tv = max(0.0, (h - s_fixed) / vcru)
    else:
        Tv = 0.0
...
def _general_displacement_for_vcru(
    vcru,
    v0,
    a0,
    vf,
    ...
    """Only compute the fixed displacement portion.

    This is considerably cheaper than constructing and replaying a
    complete profile.
    """
...
def _general_find_fastest_feasible_vcru(
    h,
    v0,
    a0,
    vf,
    ...
    """Find the largest cruise velocity whose fixed profile fits.

    The old implementation evaluated 400 points and then performed a
    40-iteration refinement.  This implementation uses a small
    deterministic scan followed by one local bisection.

    65 points is enough because the function is piecewise smooth and
    its only structural kinks are around v0 and vf.
    """

    # Explicitly include structural points.  This dramatically reduces
    # the number of scan samples required.
    base = np.linspace(
        0.0,
        vmax,
        49,
    ...
    values = np.empty_like(candidates)

    for i, vc in enumerate(candidates):
        values[i] = _general_displacement_for_vcru(
            float(vc),
            v0,
            a0,
            vf,
    ...
    feasible = values <= h + _GENERAL_FEAS_TOL

    if not np.any(feasible):
        return None

    last = int(np.max(np.nonzero(feasible)))

    # vmax itself is feasible.
    if last == len(candidates) - 1:
        return float(candidates[-1])

    lo = float(candidates[last])
    hi = float(candidates[last + 1])

    # Fast local refinement.
    for _ in range(28):
        mid = 0.5 * (lo + hi)

        s_mid = _general_displacement_for_vcru(
            mid,
            v0,
            a0,
            vf,
...
def _scurve_general_profile(
    ...
    """Optimized general asymmetric S-curve construction."""
    ...
    #
    # IMPORTANT:
    # The resulting profile is retained if it fits. We do not
    # reconstruct it later.
    # ---------------------------------------------------------

    vmax_profile = _general_profile_for_vcru(
        h,
        v0,
        a0,
        vf,
    ...
    if s_fixed <= h + _GENERAL_FEAS_TOL:
        vcru = vmax

    else:
        # -----------------------------------------------------
        # Reduced cruise velocity.
        # -----------------------------------------------------

        vcru = _general_find_fastest_feasible_vcru(
            h,
            v0,
            a0,
            vf,
        ...
        if vcru is None:
            # Infeasible inherited boundary state.
            #
            # Preserve the same safety philosophy as the original:
            # reduce the inherited state rather than emitting a
            # trajectory that violates the displacement constraint.
            lo = 0.0
            hi = 1.0
            ...
            for _ in range(24):
                ...
                s0 = _general_displacement_for_vcru(
                    0.0,
                    v0 * mid,
                    a0 * mid,
                    vf * mid,
        ...
        #
        # Build the winning profile exactly ONCE.
        # -----------------------------------------------------
    ...
    # If vmax branch was selected, its Tv was calculated using vmax.
    if vcru == vmax:
        Tv = (
            (h - s_fixed) / vcru
            if vcru > 1e-9
    ...
    # ---------------------------------------------------------
    # Materialize breakpoint states once.
    # ---------------------------------------------------------

    _, v_r1, s_r1 = _kin_ramp(
        v0,
        a0,
        apk_e,
        jmax,
    ...
    return {
        ...
        's_after_entry': s_after_entry,
        's_after_cruise': s_after_cruise,
...
def _scurve_general_eval(t, p):
    """Fast replay of a precomputed general S-curve."""
    ...
    # ---------------------------------------------------------
    # Segment 1: jerk into acceleration
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # Segment 3: jerk back to zero acceleration
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # Segment 5: jerk into exit acceleration
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # Segment 6: constant exit acceleration
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # Segment 7: jerk to final acceleration
    # ---------------------------------------------------------
...
def _scurve_general_duration(
    ...
    return _scurve_general_profile(
        h,
        v0,
        a0,
        vf,
...
def _scurve_general_time_scaled(
    h,
    T_target,
    v0,
    a0,
    ...
    """Synchronize a general profile.

    OPTIMIZATION:
    Instead of the old 40-iteration solve with a complete expensive
    profile reconstruction at every iteration, use 24 iterations.

    The final profile is constructed exactly once after convergence.

    Boundary states remain fixed.
    Only acceleration/jerk headroom is reduced.
    """
    ...
    # Inherited accelerations cannot be made unreachable.
    floor = max(
        abs(a0),
        abs(af),
    ) / max(amax, 1e-12)
    ...
    # ---------------------------------------------------------
    # Duration evaluation cache.
    #
    # The same scale can occasionally be reached again because
    # of floating-point midpoint convergence. Avoid reconstructing
    # the profile in that case.
    # ---------------------------------------------------------

    cache = {}

    def duration_for_scale(scale):
        ...
        profile = _scurve_general_profile(
            h,
            v0,
            a0,
            vf,
    ...
    # We want:
    #
    #   duration(scale) ~= T_target
    #
    # Lower acceleration/jerk => longer duration.
    #
    for _ in range(iters):
        mid = 0.5 * (lo + hi)

        T_mid = duration_for_scale(mid)
