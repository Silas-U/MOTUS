...
class MotionPlanner(Node):
    ...
    def __init__(self):
        ...
        # SetJointTarget — manual/direct joint control (joint_jog_server proxies
        # to this). Separate interface from ExecuteMotion (which is pose-only,
        # multi-leg, recipe-facing) but shares the same trajectory-generation
        # helper and JTC client — one place owns all of that, no duplication.
    ...
    # =========================================================
    # SHARED TRAJECTORY GENERATION (one leg at a time)
    # =========================================================
    # Used by both the multi-leg ExecuteMotion handler (recipe-driven,
    # pose-only) and the single-leg SetJointTarget handler (manual joint
    # control) — one place owns IK solving, create_trajectory calls, and
    # continuity/limit checking, so neither path can drift out of sync
    # with the other.

    def _generate_leg(self, q_seed, traj_method, traj_type, blend_radius, speed_scale,
                       target_pose_arr=None, target_joints_arr=None):
        """Returns (points, q_target, error_code). error_code: 0 ok,
        2 IK failed, 3 trajectory generation failed, 4 limit violation abort."""
    ...
    def _points_to_jtc_msg(self, points, joint_names, dt_default, force_final_zero_velocity=True):
        """Converts a (possibly concatenated, multi-leg) points list into one
        JointTrajectory message, and returns the FK path for visualization."""
        ...
        # Safety net: the true final point of the dispatched unit must end
        # at zero velocity regardless of what blend_radius produced —
        # intermediate leg boundaries keep their nonzero exit velocity for
        # genuine blending, only the very last point is forced to a stop.
    ...
    def _map_jtc_result(self, jtc_result, step_id_for_log: str) -> tuple:
        ...
        # GOAL_TOLERANCE_VIOLATED=-5) — our result fields are uint32, so
        # passing the raw value through overflows and crashes the node at
        # serialization time. Map to a fixed positive code and log JTC's
        # actual code for diagnosis instead.
    ...
    # =========================================================
    # EXECUTE MOTION ACTION SERVER (orchestrator-driven recipe steps)
    # =========================================================
    # Multi-leg: dispatches a whole detected run of consecutive MoveSteps
    # as ONE JTC goal, so only the true final leg ends at zero velocity —
    # every earlier leg blends into the next via its own blend_radius,
    # preserving real corner blending instead of stopping at every step.
    ...
    def _execute_motion_goal_cb(self, goal_request):
        ...
        if not self.active_mode_flag:
            ...
            # actual state). Reject outright rather than execute against a
            # seed we know is wrong.
            self.get_logger().error(
                'ExecuteMotion goal rejected — system_mode is not ACTIVE. '
                'Call /set_system_mode {new_mode: "ACTIVE"} before running a recipe.')
            return GoalResponse.REJECT
    ...
    async def _execute_motion_cb(self, goal_handle):
        ...
        self.get_logger().info(
            ...
            f'seeding from q_seed={q_seed.tolist()}')

        full_points = []
        leg_time_ranges = []   # (step_id, t_start, t_end)
        t_offset = 0.0
        ...
        for i in range(n_legs):
            ...
            # Force the LAST leg to a real stop regardless of what was
            # authored — only the true end of this dispatched unit should
            ...
            # Compute this leg's own duration from its ORIGINAL (un-offset)
            # time values BEFORE mutating anything — each create_trajectory
            # call starts fresh at t=0, so this is just its own last point's
            # time, but computed generically for clarity/safety.
            ...
            # Drop the duplicate boundary point for every leg after the
            # first — its first point is the same physical point as the
            # previous leg's last point (matches the original
            # build_task_trajectory concatenation pattern).
    ...
    # =========================================================
    # SET JOINT TARGET ACTION SERVER (manual/direct joint control)
    # =========================================================
    # Single leg, joint-space target. joint_jog_server proxies to this —
    # deliberately unreachable from any recipe (no Step type in steps.py
    # can produce a goal here). Shares _generate_leg/_points_to_jtc_msg/
    # _map_jtc_result with the recipe path above, so both stay consistent.
    ...
    def _set_joint_target_goal_cb(self, goal_request):
        ...
        if not self.active_mode_flag:
            self.get_logger().error(
                ...
                'Call /set_system_mode {new_mode: "ACTIVE"} first.')
            return GoalResponse.REJECT
        return GoalResponse.ACCEPT
    ...
    async def _set_joint_target_cb(self, goal_handle):
        ...
        # Manual moves always come to a full, clean stop — blend_radius=0.
